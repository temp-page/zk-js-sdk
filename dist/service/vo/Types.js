...
class BalanceAndAllowance extends Balance {
    ...
    async flushed() {
        const balanceAndAllowances = await (0, AddressInfo_1.getCurrentAddressInfo)().readonlyConnectInfo().erc20().batchGetBalanceAndAllowance(this.user, this.spender, [this.token]);
        const balanceAndAllowance = balanceAndAllowances[0];
        this.allowance = balanceAndAllowance.allowance;
        this.balance = balanceAndAllowance.balance;
