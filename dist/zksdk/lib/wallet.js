...
class Wallet extends abstract_wallet_1.AbstractWallet {
    ...
    async curveSwap(type, param) {
        if (!this.signer) {
            throw new Error('ZKSync signer is required for sending zksync transactions.');
        }
        await this.setRequiredAccountIdFromServer('Swap');
        const transactionData = {
            poolAccountId: param.poolAccountId,
            accountId: this.accountId,
            accountAddress: this.address(),
        ...
        let result;
        if (type === 'base') {
            result = await this.signer.signCurveBaseSwap(transactionData);
        }
        else {
            result = await this.signer.signCurveCombineSwap(transactionData);
        }
        return this.provider.curveSwap(result);
    }
