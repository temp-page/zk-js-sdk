...
exports.IERC20_INTERFACE = new ethers_1.utils.Interface(IERC20_json_1.default.abi);
exports.SYNC_MAIN_CONTRACT_INTERFACE = new ethers_1.utils.Interface(SyncMain_json_1.default.abi);
exports.SYNC_GOV_CONTRACT_INTERFACE = new ethers_1.utils.Interface(SyncGov_json_1.default.abi);
exports.SYNC_NFT_FACTORY_INTERFACE = new ethers_1.utils.Interface(NFTFactory_json_1.default.abi);
exports.IEIP1271_INTERFACE = new ethers_1.utils.Interface(IEIP1271_json_1.default.abi);
exports.MULTICALL_INTERFACE = new ethers_1.utils.Interface(Multicall_json_1.default.abi);
...
async function signMessagePersonalAPI(signer, message) {
    if (signer instanceof ethers_1.ethers.providers.JsonRpcSigner) {
        return signer.provider.send('personal_sign', [ethers_1.utils.hexlify(message), await signer.getAddress()]).then((sign) => sign, (err) => {
...
async function verifyERC1271Signature(address, message, signature, signerOrProvider, messagePrefixed = true) {
...
async function getEthSignatureType(_provider, message, signature, address) {
...
function serializeAmountPacked(amount) {
    return packAmount(ethers_1.BigNumber.from(amount));
}
...
async function serializeSwap(swap) {
    ...
    const ordersHashed = await (0, crypto_1.rescueHashOrders)(ethers_1.ethers.utils.concat([orderA, orderB]));
...
function serializeWithdraw(withdraw) {
    const type = new Uint8Array([255 - 2]);
    const version = new Uint8Array([exports.CURRENT_TX_VERSION]);
...
function serializeAddLiquidity(addLiquidity) {
    // https://github.com/zkdex-perpetual/zksync/blob/spot/core/lib/types/src/tx/add_liquidity.rs#L128
    const items = [];
    items.push(new Uint8Array([255 - 5]));
    items.push(new Uint8Array([exports.CURRENT_TX_VERSION]));
    addLiquidity.tokenIds.forEach((tokenId) => {
        items.push(serializeTokenId(tokenId));
    });
    items.push(serializeTokenId(addLiquidity.lpTokenId));
    items.push(serializeAccountId(addLiquidity.poolAccountId));
    items.push(serializeAddress(addLiquidity.accountAddress));
    items.push(serializeAccountId(addLiquidity.accountId));
    items.push(serializeNonce(addLiquidity.nonce));
    items.push(serializeAmountPacked(addLiquidity.lpAmountMin));
    addLiquidity.amounts.forEach((amount) => {
        items.push(serializeAmountPacked(amount));
    });
    return ethers_1.ethers.utils.concat(items);
}
exports.serializeAddLiquidity = serializeAddLiquidity;
function serializeRemoveLiquidity(removeLiquidity) {
    // https://github.com/zkdex-perpetual/zksync/blob/spot/core/lib/types/src/tx/remove_liquidity.rs#L111
    const items = [];
    items.push(new Uint8Array([255 - 6]));
    items.push(new Uint8Array([exports.CURRENT_TX_VERSION]));
    removeLiquidity.tokenIds.forEach((tokenId) => {
        items.push(serializeTokenId(tokenId));
    });
    items.push(serializeTokenId(removeLiquidity.lpTokenId));
    items.push(serializeAccountId(removeLiquidity.poolAccountId));
    items.push(serializeAddress(removeLiquidity.accountAddress));
    items.push(serializeAccountId(removeLiquidity.accountId));
    items.push(serializeNonce(removeLiquidity.nonce));
    items.push(serializeAmountPacked(removeLiquidity.lpAmount));
    items.push(serializeAmountPacked(removeLiquidity.lpAmountMax));
    removeLiquidity.amounts.forEach((amount) => {
        items.push(serializeAmountPacked(amount));
    });
    removeLiquidity.amountsMin.forEach((amountMin) => {
        items.push(serializeAmountPacked(amountMin));
    });
    return ethers_1.ethers.utils.concat(items);
}
exports.serializeRemoveLiquidity = serializeRemoveLiquidity;
function serializeCurveBaseSwap(curveBaseSwap) {
    // https://github.com/zkdex-perpetual/zksync/blob/spot/core/lib/types/src/tx/curve_base_swap.rs#L145
    const items = [];
    items.push(new Uint8Array([255 - 7]));
    items.push(new Uint8Array([exports.CURRENT_TX_VERSION]));
    items.push(serializeTokenId(curveBaseSwap.tokenIn));
    items.push(serializeTokenId(curveBaseSwap.tokenOut));
    items.push(serializeAccountId(curveBaseSwap.poolAccountId));
    items.push(serializeAddress(curveBaseSwap.accountAddress));
    items.push(serializeAccountId(curveBaseSwap.accountId));
    items.push(serializeNonce(curveBaseSwap.nonce));
    items.push(serializeAmountPacked(curveBaseSwap.vin));
    items.push(serializeAmountPacked(curveBaseSwap.voutMin));
    return ethers_1.ethers.utils.concat(items);
}
exports.serializeCurveBaseSwap = serializeCurveBaseSwap;
function serializeCurveCombineSwap(curveCombineSwap) {
    // https://github.com/zkdex-perpetual/zksync/blob/spot/core/lib/types/src/tx/curve_combine_swap.rs#L90
    const items = [];
    items.push(new Uint8Array([255 - 8]));
    items.push(new Uint8Array([exports.CURRENT_TX_VERSION]));
    items.push(serializeTokenId(curveCombineSwap.tokenIn));
    items.push(serializeTokenId(curveCombineSwap.tokenOut));
    items.push(serializeAccountId(curveCombineSwap.poolAccountId));
    items.push(serializeAddress(curveCombineSwap.accountAddress));
    items.push(serializeAccountId(curveCombineSwap.accountId));
    items.push(serializeNonce(curveCombineSwap.nonce));
    items.push(serializeAmountPacked(curveCombineSwap.vin));
    items.push(serializeAmountPacked(curveCombineSwap.voutMin));
    return ethers_1.ethers.utils.concat(items);
}
exports.serializeCurveCombineSwap = serializeCurveCombineSwap;
...
async function getEthereumBalance(ethProvider, syncProvider, address, token) {
    let balance;
    if (isTokenETH(token)) {
...
async function getPendingBalance(ethProvider, syncProvider, address, token) {
...
async function getTxHash(tx) {
    ...
    const txBytes = await serializeTx(tx);
...
function changePubKeyEIP712Types() {
    return {
        ChangePubKey: [
...
exports.changePubKeyEIP712Types = changePubKeyEIP712Types;
